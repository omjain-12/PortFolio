...
export function LinkButton({ href, children, variant = "secondary", external = false }: LinkButtonProps) {
  ...
  if (variant === "primary") {
    variantClass = "bg-accent-gradient text-white shadow-lg shadow-blue-500/20 hover:brightness-110";
  } else if (variant === "ghost") {
    variantClass = "border border-transparent bg-transparent text-slate-300 hover:border-border hover:text-slate-100";
  } else {
    variantClass = "border border-border bg-slate-900/50 text-slate-100 hover:border-cyan-300/50 hover:bg-slate-900/80";
  }
